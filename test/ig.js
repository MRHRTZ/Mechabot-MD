...
const sesid = '50545840339%3ARdZR23LgGj7aDc%3A20'
    ...
    return new Promise(function (resolve, reject) {
        ...
        Axios.get('https://www.instagram.com/p/' + code + '/?__a=1', {
            ...
            if (post.video_versions) {
                ...
                if (post.carousel_media[0].video_versions) {
                    for (let i = 0; i < post.carousel_media.length; i++) {
                        if (post.carousel_media[i].video_versions) {
                            url.push({
                                url: post.carousel_media[i].video_versions[0].url,
                                type: 'mp4'
                            })
                        } else {
                            url.push({
                                url: post.carousel_media[i].image_versions2.candidates[0].url,
                                type: 'jpg'
                            })
                        }
                    }
                } else {
                    url = post.carousel_media.map(v => {
                        return {
            ...
            resolve({
                media_id: post.id,
                shortcode: post.code,
                capt: post.caption.text,
                urls: url,
                owner_user: post.user.username,
                date: post.taken_at,
            })
        });
    });
...
module.exports.getPost2 = getPost2
module.exports.searchUser = searchUser
// CSR2HrXIaXU
// CYvsiqhPRG8
// CDwT4XtnvpG
// CWaYoH4IzAG
getPost2('CQypWfkKdlZ')
.then(console.log)
.catch(console.log)
